...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    assert lt(sigmoid(a), 1.0 + EPS)
    assert lt(-sigmoid(a), 0.0 + EPS)
    assert_close(1 - sigmoid(a), sigmoid(-a))
    assert_close(sigmoid(0), 0.5)
    assert lt(sigmoid(a), sigmoid(a + 0.01) + EPS)
...
def test_other(a: float) -> None:
    ...
    """
    assert eq(mul(a, 0), 0)
    assert eq(mul(a, -1), neg(a))
    assert eq(add(a, 0), a)
    assert eq(add(a, neg(a)), 0)
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    assert_close(naive_sum, sum(ls1) + sum(ls2))
